          ...
          {activeMode === "weight" && (
            <View style={themed($actionSection)}>
              ...
              {weightHistory.length >= 2 && (
                <WeightGraph records={weightHistory} themed={themed} theme={theme} />
              )}
...
function WeightGraph({ records, themed, theme }: { records: WeightRecord[]; themed: any; theme: any }) {
  // Sort oldest → newest
  ...
  return (
    <View style={themed($graphCard)}>
      <View style={themed($graphHeader)}>
      ...
      {/* Graph area with line chart simulation */}
      <View style={themed($graphArea)}>
        ...
        {/* Bars */}
        <View style={themed($graphBars)}>
          {sorted.map((r, i) => {
            const pct = range > 0 ? ((r.weightKg - min) / range) * 100 : 50
            const height = Math.max(pct * 0.7 + 15, 15)
            const isLatest = i === sorted.length - 1
            return (
              <View key={r.id || i} style={themed($graphBarCol)}>
                <Text text={`${r.weightKg}`} size="xxs" preset={isLatest ? "bold" : "default"} style={isLatest ? { color: "#4A8C3F" } : themed($dimText)} />
                <View
                  style={[
                    themed($graphBar),
                    {
...
// Weight graph styles
const $graphCard: ThemedStyle<ViewStyle> = ({ colors, spacing }) => ({
  backgroundColor: colors.palette.neutral100,
  borderRadius: 14,
  padding: spacing.md,
})

const $graphHeader: ThemedStyle<ViewStyle> = ({ spacing }) => ({
  flexDirection: "row",
  justifyContent: "space-between",
  alignItems: "center",
  marginBottom: spacing.sm,
...
const $graphBars: ThemedStyle<ViewStyle> = ({ spacing }) => ({
  flex: 1,
  flexDirection: "row",
  alignItems: "flex-end",
  gap: spacing.xxs,
...
const $graphBarCol: ThemedStyle<ViewStyle> = () => ({
  flex: 1,
  alignItems: "center",
...
const $graphBar: ThemedStyle<ViewStyle> = () => ({
  width: "70%",
  borderRadius: 4,
  minHeight: 4,
})

const $graphFooter: ThemedStyle<ViewStyle> = ({ spacing }) => ({
  flexDirection: "row",
