-- Fix date field casting in sync_push function
-- Change date_of_birth, breeding_date, etc. from ::date to ::timestamptz
-- to match the actual column types in the database
...
BEGIN
  ...
  -- Process animals - FIXED: date_of_birth now casts to timestamptz instead of date
  IF changes ? 'animals' THEN
    FOR record IN SELECT * FROM jsonb_array_elements(COALESCE(changes->'animals'->'created', '[]'::jsonb) || COALESCE(changes->'animals'->'updated', '[]'::jsonb)) LOOP
      BEGIN
        INSERT INTO animals (id, organization_id, name, visual_tag, rfid_tag, breed, sex, date_of_birth, status, dam_id, sire_id, current_pasture_id, notes, created_at, updated_at, is_deleted, remote_id)
        VALUES (
          (record->>'id'),
          (record->>'organization_id'),
          (record->>'name'),
  ...
  -- Process breeding_records - FIXED: All date fields now cast to timestamptz
  ...
  -- Process pasture_movements - FIXED: movement_date now casts to timestamptz
