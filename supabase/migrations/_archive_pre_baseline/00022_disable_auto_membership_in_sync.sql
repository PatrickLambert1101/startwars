-- Disable auto_add_org_owner trigger during sync_push execution
-- This prevents duplicate/conflicting membership creation since sync_push handles it explicitly
...
DECLARE
  ...
  errors JSONB := '[]'::jsonb;
BEGIN
  -- Disable the auto_add_org_owner trigger during sync to prevent conflicts
  -- (sync_push already handles memberships explicitly)
